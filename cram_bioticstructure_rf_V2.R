...
mydf <- cram_df %>%
  select(stationcode, COMID, bioticstructure) %>% 
...
myrf <- randomForest(y = rf_dat$bioticstructure, # dependent variable
  x = rf_dat %>%
    select(-bioticstructure), # selecting all predictor variables
...
nottrain_prediction <- nottrain %>% # taking all available COMIDS, that haven't been used in training
  na.omit() %>% # remove NAs
  mutate(biostructure_predicted = predict(myrf, newdata = nottrain %>% na.omit())) # using developed model (myrf), inputting predictor variables (nottrain - which contains COMIDs and associated StreamCat data) to predict output/dependent variable (biostructure_predicted a.k.a. CRAM).

# rePredict CRAM scores for training data.
mydf2_train$biostructure_predicted <- predict(myrf) # Add column of predicted CRAM values to training dataset.
...
set.seed(22)
my_rfe <- rfe(y = rf_dat$bioticstructure, # set dependent variable
              x = rf_dat %>% select(-bioticstructure), # set predictor variables
...
# Thresholds for Biotic Structure based on 81 reference sites in eCRAM or sent to Lisa Fong
# Very likely altered: mean < 40
# Likely altered: mean < 55
...
nottrain_prediction2 <- nottrain %>% # taking all COMIDS that haven't been used in training
  na.omit() %>% # remove NAs
  mutate(biostructure_predicted = predict(myrf2, newdata = nottrain %>% na.omit())) # using developed model (myrf2), inputting predictor variables (nottrain - COMIDs and associated StreamCat data) to predict output/dependent variable (biostructure_predicted a.k.a. CRAM).
...
#   mutate(classification = case_when(biostructure_predicted < 50 ~"Very Likely Altered",
#     biostructure_predicted < 75 ~"Likely Altered",
#     biostructure_predicted < 90 ~"Possibly Altered",
#     biostructure_predicted >= 90 ~"Likely Unaltered")) %>%
...
# ca_predictions2 <- ca_predictions2 %>%
#   mutate(classification = case_when(round(biostructure_predicted, digits = 0) < 40 ~"Very Likely Altered",
#                                     round(biostructure_predicted, digits = 0) < 55 ~"Likely Altered",
#                                     round(biostructure_predicted, digits = 0) < 66 ~"Possibly Altered",
#                                     round(biostructure_predicted, digits = 0) >= 66 ~"Likely Unaltered")) %>%
#   mutate(class_f = factor(classification, levels = c("Very Likely Altered", "Likely Altered", "Possibly Altered", "Likely Unaltered"))) # relevel classifications

# 1 threshold & 2 categories
ca_predictions2 <- ca_predictions2 %>%
  mutate(classification = case_when(round(biostructure_predicted, digits = 0) < 55 ~"Degraded",
                                    round(biostructure_predicted, digits = 0) >= 55 ~"Intact")) %>%
...
lm4 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "Central_Valley") %>%
    filter(set_f == "Testing"))

lm5 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "Chaparral") %>%
    filter(set_f == "Training"))

lm6 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "Chaparral") %>%
    filter(set_f == "Testing"))

lm7 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "Deserts_Modoc") %>%
    filter(set_f == "Training"))

lm8 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "Deserts_Modoc") %>%
    filter(set_f == "Testing"))

lm9 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "North_Coast") %>%
    filter(set_f == "Training"))

lm10 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "North_Coast") %>%
    filter(set_f == "Testing"))

lm11 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "Sierra") %>%
    filter(set_f == "Training"))

lm12 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "Sierra") %>%
    filter(set_f == "Testing"))

lm13 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "South_Coast") %>%
    filter(set_f == "Training"))

lm14 <- lm(bioticstructure~biostructure_predicted, 
  data = full_train_test %>%
    filter(PSA6 == "South_Coast") %>%
    filter(set_f == "Testing"))
...
# png(file="bioticstructure_modeled_CA.png", units="in", width=8, height=5, res=300)
# png(file="L:/RipRAM_ES/Data/Working/healthy_watershed_random_forest/Results using published thresholds/bioticstructure_modeled_CA_81ref_2.png", units="in", width=8, height=5, res=300)
# modeled_cram_map
# dev.off()
